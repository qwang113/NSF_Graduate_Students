...
for(years in years_to_pred){
  ...
  repeated_school <- do.call(rbind, replicate(n, school_idx,simplify = FALSE))
  ...
  design_here <- cbind(transformed_H, repeated_school)
  ...
  while (save_idx < total_samples) {
    ...
    DIAG <- Matrix(diag(c(rep(curr_sig_eta_inv, nh*ns), rep(curr_sig_xi_inv, N))),sparse = TRUE)
    ...
    curr_alpha_eta <- matrix(c(y_tr+eps,rep(alpha,nh*ns+N)))
    # Define current kappa_eta
    curr_kappa_eta <- c(rep(1,nrow(design_here)), rep(alpha,nh*ns+N))
    ...
    if( (curr_idx > burn) & ((curr_idx - burn)%%thin == 0) ){
      ...
      pred_here <- cbind(transformed_H_pred, school_idx)
