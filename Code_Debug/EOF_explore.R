...
w_ar <- seq(from = 1, to = 2, by = 0.1)
res <- matrix(NA, nrow = length(w_pc), ncol = length(w_ar))
for (param_pc in 1:length(w_pc)) {
  for (param_ar in 1:length(w_ar)) {
    print(paste("now doing index",param_pc,param_ar))
    leak <- 1
    year = 2012
    #Initialize
    ...
    res[param_pc,param_ar] <- mean((nsf_wide_car$X2012 - glm_pred)^2)
