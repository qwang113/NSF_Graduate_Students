  ...
  for (a_par_idx in 1:length(a_par) ) {
    for (nu_par_idx in 1:length(nu_par)) {
      ...
      for (i in 2:(year-1972+1)) {
        ...
        now_h <- matrix(NA, nrow = nrow(nsf_wide_car), ncol = nh)
        for (j in 1:nrow(nsf_wide_car)) {
          jth_school_h <- nu/lambda_scale* W %*% curr_H[nrow(curr_H)-nrow(nsf_wide_car)+j,] +
            +  t(min_max_scale(log(nsf_wide_car[,i+8]+1))[j] * ar_col) 
            + t(U_pc) %*% pc_prev_raw[i-1,] 
          now_h[j,] <- tanh(jth_school_h)
        }
        
        Y <- c(Y, nsf_wide_car[,i+9])
        curr_H <- rbind(curr_H, now_h)
      }
