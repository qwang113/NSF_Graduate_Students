...
nh <- 10 # Number of hidden units in RNN
...
for (year in 2012:2021) {
  ...
  for (curr_sch in 1:nrow(nsf_wide_car)) {
    curr_obs_H <- obs_H[curr_sch+nrow(nsf_wide_car)*((0:(year-1972-1))),]
    curr_obs_y <- obs_y[curr_sch+nrow(nsf_wide_car)*((0:(year-1972-1)))]
    glm_model <- glm.nb(curr_obs_y ~ ., data = as.data.frame(cbind(curr_obs_y, curr_obs_H)))
    glm_pred <- predict(glm_model, type = "response", newdata = as.data.frame(pred_H)[curr_sch,])
    one_step_ahead_pred_y[curr_sch,year-2011] <- glm_pred
  }
  

}

# To add ensemble, make predictions an three dimensional array, and we could run it parallel.
