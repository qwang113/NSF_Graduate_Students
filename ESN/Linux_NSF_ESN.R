...
nsf_wide <- read.csv("/hb/home/qwang113/nsf_grad/nsf_final_wide.csv", header = TRUE)
nsf_long <- read.csv("/hb/home/qwang113/nsf_grad/nsf_final_long.csv", header = TRUE)
...
Y <- nsf_wide[,4]
# curr_model <- glm.nb(Y~curr_H, control = glm.control(epsilon = 1e-8, maxit = 100, trace = TRUE))
# pred_Y <- predict(curr_model)
# curr_res <- Y-pred_Y
...
write.csv(res_stack, "/hb/home/qwang113/nsf_grad//nsf_dat/300filters_res3000_complex+univ_added+norm01.csv", row.names = FALSE)
