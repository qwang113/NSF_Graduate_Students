...
for (i in 2:time_step) {
  setTxtProgressBar(pb,i)
  new_H <- apply( nu/lambda_scale*curr_H[(nrow(curr_H)-nrow(nsf_wide)+1):nrow(curr_H),]%*%W + ux_dummy + ux_sp + log(nsf_wide[,i+4] + 1)%*%ar_col, c(1,2), tanh)
  Y <- c(Y, nsf_wide[,i+5])
...
one_step_ahead_pred_y <- exp(one_step_ahead_pred_y)
write.csv( as.data.frame(one_step_ahead_pred_y), "D:/77/UCSC/study/Research/temp/NSF_dat/cresn_pred_20240113.csv", row.names = FALSE)
