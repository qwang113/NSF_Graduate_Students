...
for (pca_id in unique(nsf_wide_car$UNITID)  ) {
  ...
  if( length(curr_college) != 1 ){# If there is only one school, no need to do pca for it
    ...
    # Log scale PCA
    prev_pc_res <- prcomp(t(nsf_y[curr_college ,1:(curr_year-1972)]), center = FALSE)
    num_pc <- which(cumsum(prev_pc_res$sdev^2)/sum(prev_pc_res$sdev^2) > 0.8)[1]
    curr_pc <- as.matrix(prev_pc_res$x[,1:num_pc])
    curr_pc_all <- array(0, dim = c(nrow(nsf_y),curr_year-1972,num_pc))
    curr_pred_pc_all <- array(0, dim = c(nrow(nsf_y),1,num_pc))
    for (i in 2:(curr_year-1972)) {
      curr_pc_all[curr_college,i,] <- matrix( rep(curr_pc[i-1,],length(curr_college)), ncol = num_pc, byrow = TRUE)
    }
    curr_pred_pc_all[curr_college,1,] <- matrix(rep(curr_pc[curr_year-1972,],length(curr_college)), ncol = num_pc, byrow = TRUE)
