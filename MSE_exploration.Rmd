...
nsf_wide <- read.csv("D:/77/UCSC/study/Research/temp/NSF_dat/nsf_final_wide.csv", header = TRUE)[,-1]

y <- nsf_wide[,(ncol(nsf_wide)-21):ncol(nsf_wide)]

colnames(one_step_pred) <- 2000:2021
colnames(one_step_res) <- 2000:2021
colnames(y) <- 2000:2021
```
```{r}
ggplot() +
  geom_point(aes(x = 2000:2021, y = apply(one_step_res^2,2,mean) )) +
  # coord_cartesian(ylim = c(250, 1500)) + 
  labs(y = "MSE", x = "year")

ggplot() +
  geom_point(aes(x = unlist(as.vector(y)), y = unlist(as.vector(one_step_res))))

```

```{r}
var(unlist(as.vector(y)))
var(unlist(as.vector(one_step_pred)))
```
