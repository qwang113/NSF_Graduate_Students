...
# Initialization
pred_all_single_esn <- matrix(NA, nrow = nrow(schoolsM), ncol = ncol(schoolsM))
...
ns <- dim(state_idx)[2]
y_tr <- as.vector(Yin)
pred_all_single_esn <- rep(NA, length(schoolsM))
# Frequentist - Integrated Model (Single)----------------------------------------------------------------------------------------  
for (school_idx in 1:nrow(schoolsM)) {
  # print(paste(years,school_idx,"Single ESN"))
  curr_idx_h <- school_idx + (0:(50 - 1))*nrow(schoolsM)
  curr_H <- H$train_h[curr_idx_h,]
  curr_y <- y_tr[curr_idx_h]
  fit_fesn <- glmnet(x = curr_H, y = curr_y, family = "poisson", alpha = 1, lambda = 1)
  pred_all_single_esn[curr_idx_h] <- predict(fit_fesn, newx = curr_H, type = "response") 
}
pred_mean <- matrix(pred_all_single_esn, nrow = nrow(schoolsM))
pred_res <- pred_mean - schoolsM

st <- pred_res/sqrt(pred_mean)
plot(st[1,],type  = 'l', pch = 19)
plot(st[10,],type  = 'l', pch = 19)
plot(st[20,], type = 'l')
plot(st[500,], type ='l')
st_var
